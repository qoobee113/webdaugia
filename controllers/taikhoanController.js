...
taikhoanr.post('/dangky',function(req, res){
    ...
    var entity = {
        ...
        sdt: req.body.sdt,
        diachi: req.body.dc,
        ngay: moment(new Date(Date.now())).format('YYYY-MM-DD HH:mm:s'),
        chucvu: 0,
        xacthuc:0,
        chan : 0
    };
...
taikhoanr.post('/chanuser', function(req, res) {
    if(req.session.isLogged===true) {
        if (req.session.isQL === true) {
            var entity =
                {
            ...
            console.log(entity);
            taikhoan.channguoidung(entity).then(function (affectedRows) {
                res.redirect('/quanliuser');
            })
        }
        else{
...
taikhoanr.post('/bochanuser', function(req, res) {
    if(req.session.isLogged===true) {
        if (req.session.isQL === true) {
            var entity =
                {
            ...
            console.log(entity);
            taikhoan.bochannguoidung(entity).then(function (affectedRows) {
                res.redirect('/quanliuser');
            })
        }
        else{
...
taikhoanr.get('/spconhan', function(req, res) {
    var entity={
        id:req.session.user.id
    }
    q.all([taikhoan.loadspconhan(entity)]).spread(function (rows) {
        ...
        res.render('Tài khoản/sanphamconhannew',vm);

    })

});
taikhoanr.get('/sanphamconguoimua', function(req, res) {
    var entity={
        id:req.session.user.id
    }
    q.all([taikhoan.loadspconguoimua(entity)]).spread(function (rows) {

        var vm={
            sanphamconguoimua:rows,
            NoProduct:rows.length === 0
        }
        console.log(rows);

        res.render('Tài khoản/sanphamconguoimuanew',vm);
